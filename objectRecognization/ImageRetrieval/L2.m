function L2(fileNames,numOfReturnedImages, queryImageFeatureVector,...
    dataset, metric)
% input:
...
% query_img=imread(fileNames{query_img_name});
% subplot(3, 7, 1);
% imshow(query_img, []);
% title('Query Image', 'Color', [1 0 0]);

% dispaly images returned by query
numOfReturnedImages=min(numOfReturnedImages,length(sortedEuclidImgs));
...
%     returned_img = imread(str_img_name);
    returned_img=imread(fileNames{img_name});
    subplot(3, 7, m+1);
    imshow(returned_img, []);
end
