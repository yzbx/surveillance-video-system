...
if __name__ == "__main__":
    ...
    if not os.path.isdir(pos_im_path):
        print "Path to positive images is invalid for {}".format(pos_im_path)
        exit()

    if not os.path.isdir(neg_im_path):
        print "Path to negative images is invalid for {}".format(neg_im_path)
        exit()
    ...
    for im_path in glob.glob(os.path.join(pos_im_path, "pos*")):
    ...
    for im_path in glob.glob(os.path.join(neg_im_path, "neg*")):
        im = imread(im_path, as_grey=True)
        if des_type == "HOG":
            fd = hog(im,  orientations, pixels_per_cell, cells_per_block, visualize, normalize)
        fd_name = os.path.split(im_path)[1].split(".")[0] + ".feat"
